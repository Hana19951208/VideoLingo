...
7. 优先复用项目现有的 `uv` / `.venv` 运行环境和旧 Streamlit 已有依赖，不要默认向系统 Python 全局补装依赖；如果控制面要启动后端或跑验证，应尽量显式使用仓库内虚拟环境。
...
### 后端控制面
```powershell
D:\Codes\VideoLingo\.venv\Scripts\python.exe -m control_plane
```
